...
class SigninForm(tk.Tk):
    ...
    def validate(username, password):
        try:
            with open(os.path.abspath("users") + '\\users.txt', 'r') as credentials:
...
class SignupForm(tk.Tk):
    ...
    def signup(self, username, password):
        ...
        if not validation:
            ...
            credentials = open(os.path.abspath("users") + '\\users.txt', 'a')
            credentials.write(f'{username}:{password}' + '\n')
            credentials.close()

            rsa_obj = RSA()
            path = os.path.join(os.path.abspath("users"), username)
            os.mkdir(path)
...
class MainForm(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        dice_img = Image.open(os.path.abspath("resources") + '\\dice.png')
        dice_img = dice_img.resize((30, 30), Image.LANCZOS)
        global ph_dice_img
        ...
        env_closed_img = Image.open(os.path.abspath("resources") + '\\envelope_closed.png')
        env_closed_img = env_closed_img.resize((100, 100), Image.LANCZOS)
        ...
        env_opened_img = Image.open(os.path.abspath("resources") + '\\envelope_opened.png')
        env_opened_img = env_opened_img.resize((100, 100), Image.LANCZOS)
