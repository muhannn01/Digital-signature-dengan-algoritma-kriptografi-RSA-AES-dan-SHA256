...
def create_envelope(sender_name, receiver_public_key_file, session_key, filename):
    try:
        ...
        path = os.path.join(os.path.abspath("") + '\\users\\', sender_name)
        ...
        with open(receiver_public_key_file, 'r') as pkey_file:
            ...
            print(repr(raw_key[0]))  # Print raw_key[0] with surrounding quotes to visualize any leading/trailing whitespace
            print("Raw key 1:", repr(raw_key[1]))
...
def open_envelope(receiver_name, sender_public_key_path, envelope_path):
    ...
    with open(os.path.abspath("") + '\\' + envelope_file_name + '.file.enc', 'wb') as f:
        f.write(encrypted_file_lines)

    path = os.path.join(os.path.abspath("") + '\\users\\', receiver_name)
    ...
    decrypted_file_name = os.path.abspath("") + '\\[' + envelope_file_name + ']' + receiver_name + '`s decrypted_file.dec'

    aes_obj = AES(decrypted_session_key)
    aes_obj.decrypt_file(inputf=os.path.abspath("") + '\\' + os.path.basename(envelope_path) + '.file.enc',
